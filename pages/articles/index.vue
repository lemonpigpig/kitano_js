        ...
        <div class="Algolia__Search__Form" role="form">
          ...
          <div class="Algolia__Search__Form__Search-icon">
            <svg height="20px" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
              <title>search</title>
              <path d="M11.192 12.606C10.024 13.482 8.572 14 7 14c-3.866 0-7-3.134-7-7s3.134-7 7-7 7 3.134 7 7c0 1.572-.518 3.024-1.394 4.192l1.41 1.41c.666-.18 1.406-.01 1.932.518l2.832 2.83c.78.78.785 2.042-.002 2.828-.78.78-2.04.787-2.827.002l-2.83-2.832c-.524-.524-.698-1.264-.518-1.932l-1.41-1.41zM7 12c2.76 0 5-2.24 5-5S9.76 2 7 2 2 4.24 2 7s2.24 5 5 5z" fill-rule="evenodd"/>
            </svg>
          </div>
...
<script type="text/javascript">
  ...
  const client = algolia('QY95CMNXF9', 'c82899e24dd0f553be412c84b49050b2')
  ...
  export default {
    asyncData () {
      return axios.get(process.env.articlesUrl)
        .then((r) => {
          return { articles: r.data.articles, cached: r.data.articles, error: false, query: '', results: {} }
        })
        .catch((e) => {
          return { articles: false, error: { status: '500', message: 'Could not load articles.' } }
        })
    },
    ...
    head () {
      return {
        title: 'Articles',
        meta: [{ hid: 'description', name: 'description', content: 'List of published articles' }],
    ...
    methods: {
      fetchData () {
        axios.get(process.env.articlesUrl)
          .then((r) => {
      ...
      processSrch: debounce(
        function (e) {
          ...
          this.query.length > 3 ? this.search() : this.reset() // limit search to 4 chars min
        }, 600
      ),
